...
constexpr S8 F32ToSNorm(F32 x) {
    const F32 nx = std::clamp(x, -1.0f, 1.0f);
    return S8(((nx >= 0.0f) ? 127.0f : 128.0f) * nx);
}

constexpr U8 F32ToUNorm(F32 x) {
    const F32 nx = std::clamp(x, 0.0f, 1.0f);
    return U8(255.0f * nx);
}

constexpr U8 F32ToSUNorm(F32 x) {
    const F32 nx = std::clamp(x, -1.0f, 1.0f);
    return U8(128.0f + ((nx >= 0) ? 127.0f : 128.0f) * nx);
}

constexpr F32 SNormToF32(S8 x) {
    return F32(x) / ((x >= 0) ? 127.0f : 128.0f);
}

constexpr F32 UNormToF32(U8 x) {
    return F32(x) / 255.0f;
}

constexpr F32 SUNormToF32(U8 x) {
